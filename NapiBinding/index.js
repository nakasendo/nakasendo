...
pG = new nakesendoBindings.playerGroupMetaDataWrap(polydegree, mod);
console.log(pG.printPlayerGrp());

var randonNr = new BigNumber(false, modN);
var grpid = nakesendoBindings.SHA256(randonNr.toString())
console.log("Group ID set to %s", grpid);

pG.SetGrpID(grpid);
console.log("Group ID from grp object %s",pG.GetGrpID());
console.log(pG.printPlayerGrp());
...
var playersInGrp = GetPlayerList(pG);
for(let [key, value] of playersInGrp){
    console.log(key + ' = ' + value.toString());
}
console.log(pG.printPlayerGrp());

console.log("GroupMetadata test");
var GrpMetaData = new GroupMetaDataWrap();
GrpMetaData.AddUserToGroup(p.UserID, "",p.addr,p.port);
