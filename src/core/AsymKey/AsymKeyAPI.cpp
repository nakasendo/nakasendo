...
{
    AsymKey keyGen;
    return std::make_pair(keyGen.getPublicKeyHEXStr(), keyGen.getPrivateKeyHEX());
}
...
std::pair<std::string, std::string> GetKeyPairHEX(const std::string& crPrivateKeyPEM)
{
    ...
    return std::make_pair(imported_key.getPublicKeyHEXStr(), imported_key.getPrivateKeyHEX());
}