...
/* 
*  Construct a BCH Address from and exisiting address string, throw if not valid 
*  @param   Address
*/
BCHAddressImpl::BCHAddressImpl( const std::string& address )
{
    m_address = address ;
    ...
    std::runtime_error err ("BCHAddressImp::constructor - address is not valid ");
    throw err;
}


/* 
*  Construct a BCH Address from public key, and version prefix
*  @param   public key as a string representation of Hex
*  @param   enum type for version, e.g. PUBLIC_KEY_ADDRESS
*/
BCHAddressImpl::BCHAddressImpl( const std::string& publicKey, VersionPrefix version  ) 
{
    // convert to binary, do sha256 and ripemd160 
    BCHTypes::byteVecType publicKeyVec = HexToUInt( publicKey ) ;
    ...
    BCHTypes::byteVecType addressByteVec ;
...
/* 
*  Get the version prefix of a BCH Address
*  @return enum representing Version Prefix, e.g. TESTNET_SCRIPT_ADDRESS
*/
VersionPrefix BCHAddressImpl::getVersionPrefix( ) const 
{
    Base58EncDec encoder ;
    BCHTypes::byteVecType bytes ( encoder.decode( m_address ) );
...
*/
std::pair< std::string, std::string > BCHAddressImpl::getVersion( ) const
{ 
...
/* 
*  Decode the BCH Address string (via base58 decoder) 
*  @return string representation of decoded address
*/
std::string BCHAddressImpl::decode( ) const 
{
    Base58EncDec encoder ;
...
*/
bool BCHAddressImpl::valid( ) const
{
    ...
    // create a new vector with the verson bytes 
    BCHTypes::byteVecType checkSumBytes ;
    ...
    BCHTypes::byteVecType intermediateBytes ;
    ...
    BCHTypes::byteVecType::const_iterator check1, check2 ;
    ...
    BCHTypes::byteVecType versionByte ;
...
/* 
*  Output Operator - pretty-print the BCH Address, and derived information
*/
std::ostream& operator<< ( std::ostream& out, const BCHAddressImpl& address )
{

    std::pair< std::string, std::string > version = address.getVersion( ) ;

    address.getAddress( ) ;
    out << "BCHAddress:\t\t\t" << address.m_address  
