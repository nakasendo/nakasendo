...
class enumTableT
{
  public:
        ...
        : mLookupTable{table}, mDefaultValue{*table.begin()}
    {
        ...
        for(const typename LookupTable::value_type& item : mLookupTable)
        {
    ...
    {
        typename LookupTable::const_iterator it { mLookupTable.find(from) };
    ...
    {
        typename ReverseLookupTable::const_iterator it { mReverseLookupTable.find(to) };
