...
function(_wasm_get_output_file_name OutputFileName ModuleBaseName)
  if(CMAKE_CONFIGURATION_TYPES)
    set(_outputfilename "WASM${ModuleBaseName}\$<IF:\$<CONFIG:Debug>,d,>.js")
  else()
    if(${CMAKE_BUILD_TYPE} STREQUAL "Debug")
      set(_wasm_debug_postfix ${CMAKE_DEBUG_POSTFIX})
    endif()
    set(_outputfilename "WASM${ModuleBaseName}${_wasm_debug_postfix}.js")
  endif()
...
macro(build_wasm_modules)##################################################
  ...
  foreach(_module IN LISTS module_list)
    ...
    _wasm_get_output_file(JS_OUTPUT_FILE ${_module})
    ...
    set(WasmBuilderScriptName WASM${_module}-Builder.${builderextension})
    if(CMAKE_CONFIGURATION_TYPES)
      configure_file(${CMAKE_CURRENT_SOURCE_DIR}/wasm_module_builder.bat.in ${CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG}/${WasmBuilderScriptName})
      configure_file(${CMAKE_CURRENT_SOURCE_DIR}/wasm_module_builder.bat.in ${CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE}/${WasmBuilderScriptName})
      file(TO_NATIVE_PATH ${CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG}/${WasmBuilderScriptName} _WASM_BUILDER_DEBUG)
      file(TO_NATIVE_PATH ${CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE}/${WasmBuilderScriptName} _WASM_BUILDER_RELEASE)
      add_custom_command(TARGET ${WASM_TARGET_NAME} POST_BUILD COMMAND cmd /c "$<IF:$<CONFIG:Debug>,${_WASM_BUILDER_DEBUG},${_WASM_BUILDER_RELEASE}> ${WASM_BITCODE_FILE} ${JS_OUTPUT_FILE}")
    else()
      configure_file(${CMAKE_CURRENT_SOURCE_DIR}/wasm_module_builder.sh.in ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${WasmBuilderScriptName})
      add_custom_command(TARGET ${WASM_TARGET_NAME} POST_BUILD COMMAND /bin/sh ${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${WasmBuilderScriptName} ${WASM_BITCODE_FILE} ${JS_OUTPUT_FILE})
    endif()
    #install(FILES "${WASM_BITCODE_FILE}" DESTINATION "lib" COMPONENT WASMModules)
    install(FILES "${JS_OUTPUT_FILE}" DESTINATION "lib" COMPONENT WASMModules)
    #install(FILES "${WASM_BITCODE_FILE}" DESTINATION "example/javascript" COMPONENT "Examples javascript")
    install(FILES "${JS_OUTPUT_FILE}" DESTINATION "example/javascript" COMPONENT "Examples javascript")
  endforeach()
endmacro()
