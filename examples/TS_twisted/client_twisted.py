...
class ClientProtocol( pb.Referenceable ):
    ...
    def err_obj1(self, reason):
        self.myPrint("error getting object: {0}".format(reason))

    #--------------------------------------------------
    def createGroup(self, n, m):
        self.myPrint('calling remote createGroup with {0}, {1}'.format(n,m))
    ...
    def err_remote(self, reason):
        self.myPrint ("Error from server:", reason)

    def success_remote(self, result):
        self.myPrint("Operation succeeded, groupID = {}".format(result))
    ...
    def remote_invite(self, gid):
        ...
        sleep(randint(4,12))
        self.myPrint("Received invitation for group {0}, replying with Acceptance".format(gid))
    ...
    def remote_groupIsVerified( self, gid, calcType ) :
        ...
        if calcType == 'LITTLEK' :
            ...
            if  self.Player.isPresignInitiator(gid) :
                self.myPrint("calling remote_presigning for ALPHA")
                d  = self.orchestratorRef.callRemote \
    ...
    def remote_completed( self, gid ) :
        if self.Player.isPresignInitiator(gid) :
            ...
            if numberPresignsLeft > 0 :
                self.myPrint("number presign left to do = {0}".format(numberPresignsLeft))
                self.myPrint("calling remote_presigning for LITTLEK")
                d = self.orchestratorRef.callRemote \
                    ( "presigning", self.user, gid.encode(), b'LITTLEK' )  

            else: 
                self.myPrint("I was the presign initiator.  Looks like we're FINISHED!!!")
                d = self.orchestratorRef.callRemote \
    ...
    def initiateSuccessCallback( self, data ) :
        ...
        self.myPrint("starting presigning ")  
    ...
    def initiateErrorCallback( self, reason ) :
        self.myPrint ("Error:  from server: {0}".format( reason ))        
...
class StdioClientProtocol(basic.LineReceiver):
    ...
    def connectionMade(self):
        ...
        self.client = ClientProtocol( self.printToConsole )
        self.client.connect()
