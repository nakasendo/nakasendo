 ...
 */
std::pair<std::string, data> DecodeCashAddrStr(const std::string &str,
...
/// Convert some data to a 5bit representation.
 ...
 */
std::string Encode(uint8_t type, const std::string &prefix, const data &payload) {
    // Convert to 5bit representation
    std::vector<uint8_t> packed { PackAddrData(payload, type) };

    data checksum = CreateChecksum(prefix, packed);
    data combined = Cat(packed, checksum);
 ...
 * Decode a cashaddr string.
 */
std::tuple<uint8_t, std::string, std::vector<uint8_t>> Decode(const std::string& addr,
                                                              const std::string& defaultPrefix)
{
    std::string prefix;
    std::vector<uint8_t> payload;
    std::tie(prefix, payload) = DecodeCashAddrStr(addr, defaultPrefix);
    ...
    return {type, prefix, std::move(data)};
}
